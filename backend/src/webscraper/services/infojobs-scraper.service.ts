...
export class InfoJobsScraperService {
  ...
  async login(credentials: ScraperCredentials): Promise<boolean> {
    try {
      // Navigate to InfoJobs login page
      await this.humanBehaviorSimulator.navigateHumanLike(this.page, 'https://www.infojobs.com.br/login.aspx');
      ...
      console.error('InfoJobs login error:', error);
      return false;
    }
  }

  async searchJobs(params: JobSearchParams): Promise<ScrapedJob[]> {
    // Similar implementation to LinkedIn scraper, but for InfoJobs
  ...
  async applyToJob(job: ScrapedJob, user: User): Promise<ApplicationResult> {
    ...
    return {
      ...
      platform: 'infojobs',
      success: true,
      applicationId: `infojobs-${uuidv4()}`,
      applicationUrl: `https://www.infojobs.com.br/candidate/application/${uuidv4()}`,
      timestamp: new Date(),
    };
  }
  ...
  private getMockJobs(params: JobSearchParams): ScrapedJob[] {
    // Similar implementation to LinkedIn scraper, but for InfoJobs
    const companies = [
      'Nubank', 'iFood', 'Mercado Livre', 'Magazine Luiza', 'Itaú',
      'Bradesco', 'Santander', 'Petrobras', 'Vale', 'Ambev',
    ];
    
    const titles = [
      'Desenvolvedor Full Stack', 'Engenheiro de Software',
      'Desenvolvedor Frontend', 'Desenvolvedor Backend',
      'Arquiteto de Software', 'DevOps Engineer',
      'Product Manager', 'UX Designer', 'Data Scientist', 'Tech Lead',
    ];
    ...
    for (let i = 0; i < count; i++) {
      ...
      jobs.push({
        ...
        applicationUrl: `https://www.infojobs.com.br/vagas/${uuidv4()}`,
      });
    }
