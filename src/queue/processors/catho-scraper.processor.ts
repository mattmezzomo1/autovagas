...
import { CathoScraperService } from '../../scrapers/services/catho-scraper.service';
import { ScraperJobService } from '../../scrapers/services/scraper-job.service';
import { CircuitBreakerService } from '../services/circuit-breaker.service';

@Processor(QUEUE_NAMES.CATHO_SCRAPER)
export class CathoScraperProcessor {
  private readonly logger = new Logger(CathoScraperProcessor.name);
  ...
  constructor(
    ...
    this.concurrency = this.configService.get<number>('queue.concurrency.cathoScraper', 5);
  }
  ...
  async processJob(job: Job): Promise<any> {
    this.logger.log(`Processing Catho scraper job ${job.id}`);
    ...
    try {
      ...
      if (action === 'processJob' && jobId) {
        ...
        await this.cathoScraperService.processJob(jobId);
      ...
      await this.circuitBreakerService.recordSuccess('catho');
      
      return result;
    } catch (error) {
      this.logger.error(`Error processing Catho scraper job ${job.id}: ${error.message}`);
      
      // Record failure for circuit breaker
      await this.circuitBreakerService.recordFailure('catho');
