...
export const CreditCheckoutSuccess: React.FC = () => {
  ...
  // Obtém o ID da sessão e a quantidade de créditos dos parâmetros da URL
  const sessionId = searchParams.get('session_id');
  const creditAmount = searchParams.get('amount') ? parseInt(searchParams.get('amount')!) : 0;

  useEffect(() => {
    const verifyPayment = async () => {
      if (!sessionId || !creditAmount) {
        setStatus('error');
      ...
      try {
        ...
        if (paymentStatus === 'succeeded') {
          ...
          setStatus('success');
          setMessage(`Pagamento confirmado! ${creditAmount} créditos foram adicionados à sua conta.`);
    ...
    verifyPayment();
  }, [sessionId, creditAmount, profile, updateProfile]);
        ...
        <div className="max-w-2xl mx-auto bg-black/40 backdrop-blur-xl p-6 sm:p-8 rounded-3xl border border-white/20 shadow-2xl">
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold mb-4">
              ...
              {status === 'success' && 'Créditos Adicionados!'}
