...
@ApiTags('indeed-scraper')
@Controller('scrapers/indeed')
@UseGuards(JwtAuthGuard)
@ApiBearerAuth()
export class IndeedScraperController {
  constructor(
  ...
  @Post('login')
  @ApiOperation({ summary: 'Login no Indeed' })
  ...
  async login(
    ...
    return this.indeedScraperService.login(email, password, req.user.sub);
  }

  @Post('search')
  @ApiOperation({ summary: 'Buscar vagas no Indeed' })
  ...
  async searchJobs(
    ...
    return this.indeedScraperService.searchJobs(req.user.sub, keywords, location, filters);
  }

  @Get('job/:jobId')
  @ApiOperation({ summary: 'Obter detalhes de uma vaga no Indeed' })
  @ApiResponse({ status: 200, description: 'Busca de detalhes iniciada' })
  async getJobDetails(
    @Param('jobId') jobId: string,
    @Req() req: Request,
  ) {
    return this.indeedScraperService.getJobDetails(req.user.sub, jobId);
  }

  @Post('apply/:jobId')
  @ApiOperation({ summary: 'Candidatar-se a uma vaga no Indeed' })
  ...
  async applyToJob(
    ...
    return this.indeedScraperService.applyToJob(req.user.sub, jobId, coverLetter, resumeUrl);
  }
