...
@ApiTags('infojobs-scraper')
@Controller('scrapers/infojobs')
@UseGuards(JwtAuthGuard)
@ApiBearerAuth()
export class InfoJobsScraperController {
  constructor(
    private readonly infoJobsScraperService: InfoJobsScraperService,
    private readonly scraperJobService: ScraperJobService,
  ) {}

  @Post('login')
  @ApiOperation({ summary: 'Login no InfoJobs' })
  ...
  async login(
    ...
    return this.infoJobsScraperService.login(email, password, req.user.sub);
  }

  @Post('search')
  @ApiOperation({ summary: 'Buscar vagas no InfoJobs' })
  ...
  async searchJobs(
    ...
    return this.infoJobsScraperService.searchJobs(req.user.sub, keywords, location, filters);
  }

  @Get('job/:jobId')
  @ApiOperation({ summary: 'Obter detalhes de uma vaga no InfoJobs' })
  @ApiResponse({ status: 200, description: 'Busca de detalhes iniciada' })
  async getJobDetails(
    @Param('jobId') jobId: string,
    @Req() req: Request,
  ) {
    return this.infoJobsScraperService.getJobDetails(req.user.sub, jobId);
  }

  @Post('apply/:jobId')
  @ApiOperation({ summary: 'Candidatar-se a uma vaga no InfoJobs' })
  ...
  async applyToJob(
    ...
    return this.infoJobsScraperService.applyToJob(req.user.sub, jobId, coverLetter, resumeUrl);
  }
